...
describe('ForcingChainsCandidateEliminator', () => {
    it('no changes when board is fully solved', () => {
        ...
        const eliminator = new ForcingChainsCandidateEliminator()
    ...
    it('handles board with bi-value cells', () => {
        const values = new Int8Array(81)
        for (let c = 0; c < 9; c++) {
            values[c] = c + 1
        }
    ...
    it('handles boards with no bi-value cells', () => {
        const values = new Int8Array(81)
        for (let c = 0; c < 9; c++) {
            values[c] = c + 1
        }
    ...
    it('handles contradiction detection gracefully', () => {
        const values = new Int8Array(81)
        values[0] = 1
        values[1] = 2
        values[2] = 3
    ...
    it('handles elimination chain with a real puzzle', () => {
        // Use a real puzzle that has bi-value cells
        const puzzle =
            '000000000000000000000000000000123000000456000000789000000000000000000000000000000'

        const board = Board.fromValues(
            new Int8Array(puzzle.split('').map(Number)),
        )
        new SimpleCandidateEliminator().eliminate(board)
    ...
    it('eliminator has correct display name', () => {
        const eliminator = new ForcingChainsCandidateEliminator()
        expect(eliminator.displayName).toBe('Forcing Chains')
    })
})
