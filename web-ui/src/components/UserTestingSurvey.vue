...
interface SurveyQuestion {
  questionId: string
  questionText: string
  responseType: 'RATING_1_5' | 'EMOJI_RATING' | 'YES_NO' | 'MULTIPLE_CHOICE' | 'TEXT' | 'RATING_1_10'
  category: string
}
...
interface SurveyResponse {
  type: 'RatingResponse' | 'TextResponse'
  value?: number
  text?: string
}
...
const currentQuestion = computed<SurveyQuestion>(() => props.survey.questions[currentQuestionIndex.value])
const totalQuestions = computed<number>(() => props.survey.questions.length)
const progressPercentage = computed<number>(() => ((currentQuestionIndex.value + 1) / totalQuestions.value) * 100)
...
const selectEmoji = (value: string): void => {
  selectedEmoji.value = value
  saveCurrentAnswer()
}

const selectBoolean = (value: boolean): void => {
  selectedBoolean.value = value
  saveCurrentAnswer()
}

const selectMultipleChoice = (value: string): void => {
  selectedMultipleChoice.value = value
  saveCurrentAnswer()
}

const saveCurrentAnswer = (): void => {
  const question = currentQuestion.value
  if (!question) return
  ...
  switch (question.responseType) {
    ...
    case 'MULTIPLE_CHOICE':
      response = { type: 'TextResponse', text: selectedEmoji.value || String(selectedBoolean.value) || selectedMultipleChoice.value || '' }
      break
    case 'TEXT':
  ...
  responses.value[question.questionId] = response
}

const getPlaceholderText = (): string => {
  const category = currentQuestion.value.category
  if (category === 'ENGAGEMENT') return 'Tell us what you enjoyed most...'
  if (category === 'LEARNING_EFFECTIVENESS') return 'What did you learn today?'
  if (category === 'SATISFACTION') return 'Share your thoughts about the experience...'
...
const loadCurrentAnswer = (): void => {
  const question = currentQuestion.value
  if (!question) return

  const savedResponse = responses.value[question.questionId]
  if (!savedResponse) return

  switch (question.responseType) {
    case 'RATING_1_5':
      rating.value = savedResponse.value ?? 0
      break
    case 'RATING_1_10':
      rating10.value = savedResponse.value ?? null
      break
    case 'EMOJI_RATING':
    case 'YES_NO':
    case 'MULTIPLE_CHOICE':
      if (question.responseType === 'YES_NO') {
        selectedBoolean.value = savedResponse.text === 'true'
      } else {
        selectedEmoji.value = savedResponse.text ?? null
        selectedMultipleChoice.value = savedResponse.text ?? null
      }
      break
    case 'TEXT':
      textResponse.value = savedResponse.text ?? ''
      break
  }
}
...
const calculateOverallRating = (): number | null => {
  const ratingResponses = Object.values(responses.value).filter(
    (r: SurveyResponse) => r.type === 'RatingResponse'
  )
  if (ratingResponses.length === 0) return null
  const sum = ratingResponses.reduce((acc: number, r: SurveyResponse) => acc + (r.value ?? 0), 0)
